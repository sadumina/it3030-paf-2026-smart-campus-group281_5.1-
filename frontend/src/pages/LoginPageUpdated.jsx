...
export default function LoginPageUpdated() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setTimeout(() => navigate(getDashboardPathForRole(response.role)), 1000);
    } catch (requestError) {
  ...
  const handleGoogleSuccess = (response) => {
    setMessage(`Welcome, ${response.userName}! Redirecting...`);
    
    if (window.notificationCenter) {
      window.notificationCenter.addNotification(
        createNotification('Google Login Successful', {
          type: notificationTypes.SUCCESS,
    ...
    setTimeout(() => navigate(getDashboardPathForRole(response.role)), 1500);
  };
              ...
              <div className="pt-4">
                <div className="relative mb-4">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-white px-2 text-slate-500 font-semibold">Or continue with Google</span>
                  </div>
                </div>
