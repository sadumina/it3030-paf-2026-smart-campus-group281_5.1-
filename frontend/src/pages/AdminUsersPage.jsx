...
export default function AdminUsersPage() {
  ...
  const displayedUsers =
    activeTab === "privileged" ? privilegedUsers : standardUsers;
    ...
    <>
      ...
      <RoleDashboardLayout
        ...
        kpis={[
          ...
          {
            label: "Users",
            value: String(stats.regularUsers),
        ...
        activityFeed={[
          {
            title: "Use role assign to update access instantly",
            meta: "Role changes are JWT-based",
          },
          {
            title: "Delete removes account permanently",
            meta: "Use with caution",
          },
          {
            title: "Current admin cannot be deleted",
            meta: "Self-protection enabled",
          },
        ]}
          ...
          <section className="dashboard-soft-in rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {!isSuperAdmin ? (
              <div className="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700">
                Admin accounts can manage only USER and TECHNICIAN profiles.
                Privileged account management is SUPER_ADMIN only.
              </div>
            ) : null}
                  ...
                  <tbody className="divide-y divide-slate-200 bg-white">
                    {displayedUsers.map((user) => {
                      ...
                      const canManageTarget =
                        isSuperAdmin || canAdminManageRole(targetRole);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${getRoleBadgeColor((user.role || "USER").toUpperCase())}`}
                            >
                              {(user.role || "USER").toUpperCase()}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              <select
                                value={selectedRole}
                                onChange={(event) =>
                                  handleRoleDraft(user.id, event.target.value)
                                }
